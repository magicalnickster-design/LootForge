...
export function registerEntitlementSettings() {
  ...
  game.settings.register(MODULE_ID, SETTING_ENTITLEMENT_CACHE, {
    name: "LootForge Entitlement Cache",
    scope: "client",
    config: false,
    type: Object,
    default: {},
...
export function stateMessage(state = currentState) {
  switch (state) {
    ...
    case AUTH_STATES.ENTITLEMENT_EXPIRED:
      return "Your LootForge access has expired. Connect to the internet and check your subscription.";
...
export async function syncEntitlement({ notify = false, force = false } = {}) {
  ...
  if (!snapshot.ok) {
    ...
    if (notify) ui.notifications.error(`LootForge: ${snapshot?.message || stateMessage(nextState)}`);
  ...
  if (!entitlement.allowed) {
    ...
    if (notify) ui.notifications.error(`LootForge: ${stateMessage(reason)}`);
  ...
  if (!Number.isFinite(expiresMs) || expiresMs <= Date.now()) {
    ...
    if (notify) ui.notifications.error(`LootForge: ${stateMessage(AUTH_STATES.ENTITLEMENT_EXPIRED)}`);
...
export async function restoreSessionOnStartup({ notify = false } = {}) {
  ...
  await SessionStore.importSceneForgeSessionIfEmpty();
