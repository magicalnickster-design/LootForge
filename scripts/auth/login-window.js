...
export async function performLogin(formData) {
  ...
  if (!result.ok) {
    ...
    if (code === "EMAIL_NOT_VERIFIED") {
      ui.notifications.error("LootForge: Email verification required. Please verify your account.");
    } else if (code === "AUTH_REQUIRED" || code === "SESSION_EXPIRED") {
      ui.notifications.error("LootForge: Session expired. Please sign in again.");
    } else if (code === "BACKEND_UNAVAILABLE") {
      ui.notifications.error("LootForge: Backend unavailable. Please retry.");
    } else {
      ui.notifications.error(`LootForge: ${result.message || "Sign in failed."}`);
    }
    return false;
  }
  ...
  if (entitlementResult?.ok) {
    ui.notifications.info("LootForge: Signed in successfully.");
    return true;
  }
  ui.notifications.warn("LootForge: Signed in, but LootForge access could not be verified.");
  return true;
}

export function openLoginWindow({ title = "LootForge — Gambits Forge Login" } = {}) {
  const content = `
    <form class="lootforge-gambits-login">
...
export async function logoutLootForge() {
  ...
  ui.notifications.info("LootForge: Logged out.");
}
